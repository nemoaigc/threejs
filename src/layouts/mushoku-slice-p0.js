/**
 * Mushoku Tensei village-slice P0 — breathable countryside, not a jammed plaza.
 *
 * img2threejs heroes are large (temple ~14×28×23). Layout uses plant scale +
 * wide spacing so footprints never overlap. Empty meadow > clutter.
 */
export const MUSHOKU_SLICE_P0 = {
  meta: {
    name: 'mushoku-buena-roa-slice-p0',
    playableHalfExtent: 48,
    roadWidthMain: 4.6,
    plazaSize: 12,
  },

  roads: [
    // shorter cross — don't pave the whole meadow
    { id: 'road.ew', x0: -22, z0: 0, x1: 22, z1: 0, width: 4.6, step: 5 },
    { id: 'road.ns', x0: 0, z0: -36, x1: 0, z1: 18, width: 4.6, step: 5 },
  ],

  places: [
    {
      id: 'plaza.core',
      type: 'plaza',
      x: 0,
      z: 0,
      yaw: 0,
      footprintWxD: [12, 12],
      heightHint: 'S',
      zone: 'B',
      priority: 'P0',
    },

    // Three landmarks — spaced so AABB never kiss (after plant scale)
    // guild west · temple deep north · inn east-southeast
    {
      id: 'landmark.adventurers_guild',
      type: 'adventurersGuild',
      x: -22,
      z: -14,
      yaw: 0.15,
      scale: 0.82,
      footprintWxD: [11, 8],
      heightHint: 'L',
      zone: 'C',
      priority: 'P0',
    },
    {
      id: 'landmark.temple',
      type: 'temple',
      x: 0,
      z: -34,
      yaw: 0.04,
      // factory is cathedral-scale; village slice needs ~0.55
      scale: 0.55,
      footprintWxD: [8, 13],
      heightHint: 'L',
      zone: 'C',
      priority: 'P0',
    },
    {
      id: 'landmark.inn',
      type: 'inn',
      x: 22,
      z: -12,
      yaw: -0.12,
      scale: 0.88,
      footprintWxD: [9, 7],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },

    // Main-street shops (Buena/Roa slice) — spaced off heroes
    {
      id: 'shop.magic',
      type: 'shopMagic',
      x: 18,
      z: 10,
      yaw: Math.PI,
      scale: 0.95,
      footprintWxD: [5, 5],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },
    {
      id: 'shop.smithy',
      type: 'shopSmithy',
      x: -18,
      z: 10,
      yaw: Math.PI,
      scale: 0.95,
      footprintWxD: [5.5, 5],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },
    {
      id: 'shop.general',
      type: 'shopGeneral',
      x: -16,
      z: -5,
      yaw: 0.05,
      scale: 0.92,
      footprintWxD: [5, 4.5],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },
    {
      id: 'prop.carriage',
      type: 'carriageStop',
      x: 12,
      z: 5,
      yaw: Math.PI,
      scale: 1.0,
      footprintWxD: [6, 4],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },

    // Only two distant cottages — not a ring of houses
    {
      id: 'cottage.nw',
      type: 'cottageGlb',
      x: -34,
      z: 14,
      yaw: Math.PI * 0.85,
      scale: 1.05,
      footprintWxD: [5, 5],
      heightHint: 'S',
      zone: 'D',
      priority: 'P1',
    },
    {
      id: 'cottage.se',
      type: 'cottageGlb',
      x: 34,
      z: -28,
      yaw: -0.55,
      scale: 1.0,
      footprintWxD: [5, 5],
      heightHint: 'S',
      zone: 'D',
      priority: 'P1',
    },

    // ── Street dressing (batch props) — sparse, intentional clusters ──
    // Plaza: well + quest board + lanterns + bench
    {
      id: 'prop.well',
      type: 'well',
      x: -3.2,
      z: 2.5,
      yaw: 0.2,
      scale: 1,
      footprintWxD: [3.1, 3.1],
      heightHint: 'S',
      zone: 'B',
      priority: 'P0',
    },
    {
      id: 'prop.quest_board',
      type: 'questBoard',
      x: 3.5,
      z: 3.2,
      yaw: -0.4,
      scale: 1,
      footprintWxD: [1.4, 0.5],
      heightHint: 'S',
      zone: 'B',
      priority: 'P0',
    },
    {
      id: 'prop.lantern.plaza.sw',
      type: 'streetLight',
      x: -5.5,
      z: 5.5,
      yaw: 0,
      scale: 1,
      footprintWxD: [1, 1],
      heightHint: 'S',
      zone: 'B',
      priority: 'P0',
    },
    {
      id: 'prop.lantern.plaza.se',
      type: 'streetLight',
      x: 5.5,
      z: 5.5,
      yaw: Math.PI,
      scale: 1,
      footprintWxD: [1, 1],
      heightHint: 'S',
      zone: 'B',
      priority: 'P0',
    },
    {
      id: 'prop.lantern.plaza.nw',
      type: 'streetLight',
      x: -5.2,
      z: -5.2,
      yaw: 0.5,
      scale: 1,
      footprintWxD: [1, 1],
      heightHint: 'S',
      zone: 'B',
      priority: 'P0',
    },
    {
      id: 'prop.bench.plaza',
      type: 'bench',
      x: 4.2,
      z: -3.5,
      yaw: -0.6,
      scale: 1,
      footprintWxD: [1.4, 0.5],
      heightHint: 'S',
      zone: 'B',
      priority: 'P0',
    },
    {
      id: 'prop.planter.plaza',
      type: 'planter',
      x: -4.5,
      z: -3.8,
      yaw: 0.3,
      scale: 1,
      footprintWxD: [0.8, 0.5],
      heightHint: 'S',
      zone: 'B',
      priority: 'P0',
    },

    // Crossroads markers
    {
      id: 'prop.signpost.cross',
      type: 'signpost',
      x: 7.5,
      z: 7.5,
      yaw: 0.2,
      scale: 1,
      footprintWxD: [0.8, 0.8],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },
    {
      id: 'prop.waystone.n',
      type: 'waystone',
      x: 2.5,
      z: -18,
      yaw: 0.1,
      scale: 1,
      footprintWxD: [0.6, 0.5],
      heightHint: 'S',
      zone: 'C',
      priority: 'P0',
    },

    // Magic shop porch
    {
      id: 'prop.crystal.magic',
      type: 'crystalCrate',
      x: 16.2,
      z: 12.2,
      yaw: 0.4,
      scale: 1,
      footprintWxD: [0.7, 0.6],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },
    {
      id: 'prop.barrel.magic',
      type: 'barrelCluster',
      x: 20.2,
      z: 11.5,
      yaw: -0.3,
      scale: 1,
      footprintWxD: [1.2, 0.8],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },
    {
      id: 'prop.lantern.magic',
      type: 'streetLight',
      x: 15.5,
      z: 8.5,
      yaw: 0,
      scale: 1,
      footprintWxD: [1, 1],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },

    // Smithy porch
    {
      id: 'prop.anvil.smithy',
      type: 'anvilProp',
      x: -16.5,
      z: 12.2,
      yaw: 0.5,
      scale: 1,
      footprintWxD: [0.8, 0.5],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },
    {
      id: 'prop.woodpile.smithy',
      type: 'woodpile',
      x: -20.5,
      z: 11.8,
      yaw: 0.2,
      scale: 1,
      footprintWxD: [1.5, 0.8],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },
    {
      id: 'prop.lantern.smithy',
      type: 'streetLight',
      x: -15.5,
      z: 8.5,
      yaw: Math.PI,
      scale: 1,
      footprintWxD: [1, 1],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },

    // General shop porch
    {
      id: 'prop.crates.general',
      type: 'crateStack',
      x: -14.2,
      z: -2.8,
      yaw: 0.3,
      scale: 1,
      footprintWxD: [1.0, 0.8],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },
    {
      id: 'prop.sacks.general',
      type: 'sackPile',
      x: -17.8,
      z: -3.5,
      yaw: -0.2,
      scale: 1,
      footprintWxD: [2.0, 1.2],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },
    {
      id: 'prop.market.general',
      type: 'marketStall',
      x: -21.3,
      z: -3.4,
      yaw: 0.05,
      scale: 1,
      footprintWxD: [3.1, 1.7],
      heightHint: 'M',
      zone: 'A',
      priority: 'P0',
    },

    // Carriage stop edge
    {
      id: 'prop.hitch.carriage',
      type: 'hitchingPost',
      x: 14.5,
      z: 3.5,
      yaw: 0.1,
      scale: 1,
      footprintWxD: [1.5, 0.4],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },
    {
      id: 'prop.cart.road',
      type: 'handcart',
      x: 9.5,
      z: 3.2,
      yaw: -0.8,
      scale: 1,
      footprintWxD: [2.1, 3.6],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },
    {
      id: 'prop.hay.carriage',
      type: 'hayBale',
      x: 13.5,
      z: 7.2,
      yaw: 0.4,
      scale: 1,
      footprintWxD: [2.5, 1.25],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },
    {
      id: 'prop.trough.carriage',
      type: 'horseTrough',
      x: 17.8,
      z: 5.5,
      yaw: 0.05,
      scale: 1,
      footprintWxD: [4.2, 1.4],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },
    {
      id: 'prop.wagon.trade-east',
      type: 'coveredWagon',
      x: 25.8,
      z: 4.2,
      yaw: -0.08,
      scale: 1,
      footprintWxD: [5.6, 2.5],
      heightHint: 'M',
      zone: 'C',
      priority: 'P0',
    },
    {
      id: 'prop.cider-press.inn-orchard',
      type: 'ciderPress',
      x: 29.2,
      z: -13.4,
      yaw: 0.24,
      scale: 1,
      footprintWxD: [3.1, 2.8],
      heightHint: 'M',
      zone: 'C',
      priority: 'P0',
    },
    {
      id: 'prop.herb-rack.magic',
      type: 'herbRack',
      x: 23.3,
      z: 14.2,
      yaw: -0.18,
      scale: 1,
      footprintWxD: [3.2, 1.6],
      heightHint: 'M',
      zone: 'C',
      priority: 'P0',
    },
    {
      id: 'prop.beehives.east-meadow',
      type: 'beehiveCluster',
      x: 30.6,
      z: -2.1,
      yaw: 0.2,
      scale: 1,
      footprintWxD: [3.4, 1.8],
      heightHint: 'S',
      zone: 'C',
      priority: 'P0',
    },
    {
      id: 'prop.potter.northwest-cottage',
      type: 'pottersWheel',
      x: -26.7,
      z: 8.1,
      yaw: 0.16,
      scale: 1,
      footprintWxD: [3.9, 2.1],
      heightHint: 'M',
      zone: 'C',
      priority: 'P0',
    },
    {
      id: 'prop.dyeing.southwest-meadow',
      type: 'dyeingStation',
      x: -29.6,
      z: -5.0,
      yaw: -0.12,
      scale: 1,
      footprintWxD: [4.3, 2.2],
      heightHint: 'M',
      zone: 'C',
      priority: 'P0',
    },
    {
      id: 'prop.cooper.inn-workyard',
      type: 'coopersBench',
      x: 27.0,
      z: -18.7,
      yaw: 0.2,
      scale: 1,
      footprintWxD: [4.4, 2.6],
      heightHint: 'M',
      zone: 'C',
      priority: 'P0',
    },
    {
      id: 'prop.bake-oven.southeast-cottage',
      type: 'bakeOven',
      x: 25.0,
      z: -27.3,
      yaw: -0.16,
      scale: 1,
      footprintWxD: [4.8, 3.2],
      heightHint: 'M',
      zone: 'D',
      priority: 'P0',
    },

    // Guild apron — fence snippet + lantern
    {
      id: 'prop.fence.guild',
      type: 'fenceSection',
      x: -18.5,
      z: -8.5,
      yaw: 0.4,
      scale: 1,
      footprintWxD: [2.2, 0.3],
      heightHint: 'S',
      zone: 'C',
      priority: 'P0',
    },
    {
      id: 'prop.lantern.guild',
      type: 'streetLight',
      x: -17.5,
      z: -10.5,
      yaw: 0.2,
      scale: 1,
      footprintWxD: [1, 1],
      heightHint: 'S',
      zone: 'C',
      priority: 'P0',
    },
    {
      id: 'prop.bench.guild',
      type: 'bench',
      x: -19.5,
      z: -11.5,
      yaw: 0.3,
      scale: 1,
      footprintWxD: [1.4, 0.5],
      heightHint: 'S',
      zone: 'C',
      priority: 'P0',
    },

    // Inn apron
    {
      id: 'prop.barrels.inn',
      type: 'barrelCluster',
      x: 19.5,
      z: -9.5,
      yaw: 0.5,
      scale: 1,
      footprintWxD: [1.2, 0.8],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },
    {
      id: 'prop.lantern.inn',
      type: 'streetLight',
      x: 18.5,
      z: -8.0,
      yaw: -0.3,
      scale: 1,
      footprintWxD: [1, 1],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },
    {
      id: 'prop.planter.inn',
      type: 'planter',
      x: 24.0,
      z: -10.5,
      yaw: 0,
      scale: 1,
      footprintWxD: [0.8, 0.5],
      heightHint: 'S',
      zone: 'A',
      priority: 'P0',
    },

    // Sparse trees — clear pads around heroes, no mid-street clutter
    { id: 'tree.plaza.w', type: 'tree', x: -9, z: 6, yaw: 0.3, scale: 0.95, footprintWxD: [1.5, 1.5], heightHint: 'S', zone: 'A', priority: 'P0' },
    { id: 'tree.plaza.e', type: 'tree', x: 9, z: 6, yaw: 1.1, scale: 0.9, footprintWxD: [1.5, 1.5], heightHint: 'S', zone: 'A', priority: 'P0' },
    { id: 'tree.guild.sw', type: 'tree', x: -28, z: -20, yaw: 0.2, scale: 1.05, footprintWxD: [1.5, 1.5], heightHint: 'S', zone: 'C', priority: 'P0' },
    { id: 'tree.inn.ne', type: 'tree', x: 28, z: -6, yaw: 1.4, scale: 1.0, footprintWxD: [1.5, 1.5], heightHint: 'S', zone: 'A', priority: 'P0' },
    { id: 'tree.temple.w', type: 'tree', x: -12, z: -38, yaw: 0.5, scale: 1.1, footprintWxD: [1.5, 1.5], heightHint: 'S', zone: 'C', priority: 'P0' },
    { id: 'tree.temple.e', type: 'tree', x: 12, z: -38, yaw: 2.2, scale: 1.05, footprintWxD: [1.5, 1.5], heightHint: 'S', zone: 'C', priority: 'P0' },
    { id: 'tree.far.n', type: 'tree', x: -6, z: -48, yaw: 0.8, scale: 1.15, footprintWxD: [1.5, 1.5], heightHint: 'S', zone: 'D', priority: 'P1' },
    { id: 'tree.far.s', type: 'tree', x: 18, z: 16, yaw: 1.7, scale: 1.0, footprintWxD: [1.5, 1.5], heightHint: 'S', zone: 'D', priority: 'P1' },
  ],
};
